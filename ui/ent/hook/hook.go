...
// The TrafficFunc type is an adapter to allow the use of ordinary
// function as Traffic mutator.
type TrafficFunc func(context.Context, *ent.TrafficMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f TrafficFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.TrafficMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.TrafficMutation", m)
}

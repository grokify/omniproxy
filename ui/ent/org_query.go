...
func (_q *OrgQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Org, error) {
	...
	if query := _q.withProxies; query != nil {
		if err := _q.loadProxies(ctx, query, nodes,
			func(n *Org) { n.Edges.Proxies = []*Proxy{} },
			func(n *Org, e *Proxy) { n.Edges.Proxies = append(n.Edges.Proxies, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *OrgQuery) loadProxies(ctx context.Context, query *ProxyQuery, nodes []*Org, init func(*Org), assign func(*Org, *Proxy)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Org)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.Proxy(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(org.ProxiesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.org_proxies
		if fk == nil {
			return fmt.Errorf(`foreign-key "org_proxies" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "org_proxies" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil

...
func (_q *ProxyQuery) QueryTraffic() *TrafficQuery {
	query := (&TrafficClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *ProxyQuery) Clone() *ProxyQuery {
	...
	return &ProxyQuery{
		...
		withTraffic: _q.withTraffic.Clone(),
...
// WithTraffic tells the query-builder to eager-load the nodes that are connected to
// the "traffic" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *ProxyQuery) WithTraffic(opts ...func(*TrafficQuery)) *ProxyQuery {
	query := (&TrafficClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withTraffic = query
	return _q
}
...
func (_q *ProxyQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Proxy, error) {
	...
	if query := _q.withOrg; query != nil {
		if err := _q.loadOrg(ctx, query, nodes, nil,
			func(n *Proxy, e *Org) { n.Edges.Org = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withTraffic; query != nil {
		if err := _q.loadTraffic(ctx, query, nodes,
			func(n *Proxy) { n.Edges.Traffic = []*Traffic{} },
			func(n *Proxy, e *Traffic) { n.Edges.Traffic = append(n.Edges.Traffic, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *ProxyQuery) loadOrg(ctx context.Context, query *OrgQuery, nodes []*Proxy, init func(*Proxy), assign func(*Proxy, *Org)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Proxy)
	for i := range nodes {
		if nodes[i].org_proxies == nil {
			continue
		}
		fk := *nodes[i].org_proxies
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "org_proxies" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *ProxyQuery) loadTraffic(ctx context.Context, query *TrafficQuery, nodes []*Proxy, init func(*Proxy), assign func(*Proxy, *Traffic)) error {
	...
	query.withFKs = true
	query.Where(predicate.Traffic(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(proxy.TrafficColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.proxy_traffic
		if fk == nil {
			return fmt.Errorf(`foreign-key "proxy_traffic" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "proxy_traffic" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil

...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := _q.withOrg; query != nil {
		if err := _q.loadOrg(ctx, query, nodes, nil,
			func(n *User, e *Org) { n.Edges.Org = e }); err != nil {
			return nil, err
		}
	}
...
func (_q *UserQuery) loadOrg(ctx context.Context, query *OrgQuery, nodes []*User, init func(*User), assign func(*User, *Org)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*User)
	for i := range nodes {
		if nodes[i].org_users == nil {
			continue
		}
		fk := *nodes[i].org_users
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "org_users" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
